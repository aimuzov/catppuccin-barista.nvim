local blend = require("catppuccin-barista.util").color_blend
...
local override_all = function(c)
	...
	return {
		...
		SnacksDashboardKey = { bg = blend(c.base, c.crust, 15), fg = c.peach },
		SnacksDashboardKeyBorder = { bg = blend(c.base, c.crust, 15), fg = blend(c.base, c.surface1, 50) },
...
return {
	{
		...
		opts = function(_, opts)
			...
			for flavour_name, flavour_info in pairs(require("catppuccin-barista").get_flavours()) do
				flavours[flavour_name] = flavour_info.background or "dark"
			end

			for flavour_name in pairs(require("catppuccin").flavours) do
				flavours[flavour_name] = flavour_name == "latte" and "light" or "dark"
			end

			for flavour_name, flavour_type in pairs(flavours) do
				local our_override = flavour_type == "both" and override_all
					or flavour_type == "dark" and override_dark
					or override_light
				local user_override = opts.highlight_overrides[flavour_name]

				if user_override then
					opts.highlight_overrides[flavour_name] = function(c)
						return vim.tbl_deep_extend("force", user_override(c), our_override(c))
					end
				else
					opts.highlight_overrides[flavour_name] = our_override
				end
			end
		end,
